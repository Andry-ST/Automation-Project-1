...
describe('This is first test suite, Andry-Sten Tõru', () => {
    it('User can submit data only when valid mandatory values are added', () => {
        cy.get('#username').type('Makaronid123')
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('#firstName').type('Jaan')
        cy.get('#lastName').type('Sepp')
        cy.get('input[name="password"]').type('Heap2sa')
        cy.get('[name="confirm"]').type('Heap2sa')
    ...
    */
    ...
    it('User cannot add letters to phone number', () => {
        ...
        cy.get('#username').type('johnDoe')
        cy.get('[data-testid="phoneNumberTestId"]').type('10203040asd')
        cy.get('[data-testid="phoneNumberTestId"]').should('have.attr', 'type', 'number')
        cy.get("input[name='password']").type('Password123')
        cy.get('[name="confirm"]').type('Password123')
        cy.get('.submit_button').should('be.disabled')
